...
export class Tank {
  ...
  update(ts: number, moveDir: { x: number, y: number }, fireNormal: boolean, fireGrenade: boolean, cameraYaw: number = 0, cameraPitch: number = 0): { normal: boolean, grenade: boolean, muzzlePos: vec3, muzzleDir: vec3 } {
    ...
    // Create an upright version of the tank's yaw from current physics yaw
    ...
    // Cap the wind-up so if we are stuck on a wall, we don't spin wildly later
    ...
    // Apply movement along the current upright hull forward
